...
class OpenPIGPT2Predictor:
    ...
    def __init__(self, model_path: str, stop_token: str = '<|endoftext|>'):
        ...
        def t5_dict_set(path):
            dic_ = {}
            with open(os.getcwd() +path, "r", encoding="utf-8") as fr:  #t5_dict_filter_right6.tx
                for line in fr.readlines():
        ...
        self.t5_dic = t5_dict_set("/t5_dict_filter_right6_others.txt").values()
        self.t5_dic = [int(x) for x in self.t5_dic]
        self.t5_dic_source = t5_dict_set("/t5_dict_filter_right6.txt").values()
        self.t5_dic_source = [int(x) for x in self.t5_dic_source]
    ...
    def get_predictions(self, max_len, input_ctxt_and_query,
        ...
        # train_and_test_q_kg_attr =list( set(score_entity+self.t5_dic+g_attr_id))  # score_entity  self.t5_dic
        # train_and_test_q_kg_entity =list(set(score_entity+self.t5_dic+g_entity_id)) #score_entity  self.t5_dic
        train_and_test_q_kg_attr = list(set(score_entity))  # score_entity  self.t5_dic
        train_and_test_q_kg_entity = list(set(score_entity))  # score_entity  self.t5_dic
        ...
        constraint_decoder = get_constraint_decoder(tokenizer=self.tokenizer,
                                                    ...
                                                    other_position_set=self.t5_dic_source,
...
def main():
    ...
    Score_ids={}
    with open(os.getcwd() +"/test_score_right6_100.jsonl",encoding="utf-8") as f2:
        for line in f2:
            # al = al + 1
            # print("--------al", al)
